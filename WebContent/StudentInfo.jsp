...
<table id="dg" title="学生信息" class="easyui-datagrid" fitColumns="true" 
	pagination="true" rownumbers="true" fit="true" url="studentList" toolbar="#tb">
	 	<thead>
	 		<tr>
	 			...
	 			<th field="gradeId" width="50" align="center" hidden="true">班级编号</th>
   ...
   <div id="tb">
		...
		&nbsp;姓名:&nbsp;<input type="text" name="s_stuName" id="s_stuName" size="8"/>
		&nbsp;性别:&nbsp;<select class="easyui-combobox" id="s_sex" name="s_sex" editable="false" panelHeight="auto">
			<option value="">请选择...</option>
			<option value="男">男</option>
			<option value="女">女</option>
		</select>
		&nbsp;出生日期:&nbsp;<input class="easyui-datebox" name="s_bbirthday" id="s_bbirthday"  editable="false"/>-
		<input class="easyui-datebox" name="s_ebirthday" id="s_ebirthday"  editable="false"/>
		&nbsp;所属班级:&nbsp;<input class="easyui-combobox" id="s_gradeId" name="s_gradeId" editable="false" data-options="valueField:'id',textField:'gradeName',url:'gradeComboList'">
	...
	<div id="dlg" class="easyui-dialog" style="width:570px;height:350px;padding:10px 20px;" 
		...
		<form id="fm" method="post">
			<table cellspace="5px;">
				...
				<tr>
					<td>班级名称:</td>
					<td><input class="easyui-combobox" id="gradeId" name="gradeId" editable="false" data-options="valueField:'id',textField:'gradeName',url:'gradeComboList'"></td>
					<td></td>
					<td>Email:</td>
	...
	<script type="text/javascript">
		...
		function deleteStudent(){
			...
			var ids=strIds.join(",");
			alert(ids);
			$.messager.confirm("系统提示","你确认要删除这<font color='red' size='5'>"+selectedRows.length+"</font>条数据吗?",
			  		function(r){
						if(r){
			  				$.post("studentDelete",{delIds:ids},function(result){
			  				    if(result.success){
			  				    	$.messager.alert("系统提示","你已经成功删除<font color='red' size='5'>"+result.delNums+"</font>条数据");
			  				    	$("#dg").datagrid("reload");
			  				    }else{
			  				    	alert("delete error");
			  				    }
